...
def run_polar_sim_with_len_k(config_path="configs/config_polar.json5", len_k_override=512, seed=42, save_output=True):
    np.random.seed(seed)
    ...
    with redirect_stdout(terminal_output):
        ...
        config["code"]["len_k"] = len_k_override  # Inject user value
        ...
        channel = ChannelAWGN(config["channel"], seed)
        ...
        sim.save_output = int(save_output)  # Assuming your class uses int(1/0)
