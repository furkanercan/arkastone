...
class CRCEncoder:
    def __init__(self, config: CRCConfig):
        ...
        if self.mode == '5g':
            self.crc_poly, self.crc_poly_bin = self._instantiate_crcs_5g(self.crc_length)
        elif self.mode == 'generic':
            self.crc_poly, self.crc_poly_bin = self._instantiate_crcs_generic(self.crc_length)
        else:
